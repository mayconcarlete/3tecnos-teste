...
const makeSut = (): SutTypes => {
  ...
  const sut = new PrefeituraController(mockValidator, mockCreatePrefeitura)
...
describe('Prefeitura Controller', () => {
  ...
  test('Should return bad request if prefeitura already exists in DB', async () => {
    ...
    const result = await sut.handle(req)
    expect(result.statusCode).toBe(400)
    expect(result.body).toEqual(new Error('Prefeitura already exists in DB'))
  })
